...
Создадим новый проект: *django-admin startproject <name_of_project> .* - точка установит проект в данную папку, уберёт лишнюю вложенность из проекта.

Установим необходимые приложения в *python manage.py startapp*.
...
Создаём класс сериализатор __RegisterSerializer__ для обратбки регистрации пользователей. Далее класс представления __RegisterView__ для обратботки регистрации, создания токенов и отправки письма подтверждени я на почту, которое в себе представлет ссылку на подтверждение с __access token'ом__ только что зарегистрированного пользователя.

Отправка сообщения на почту реализова только средствами __Django__ и дополнительным функционалом БЕЗ __Djoser__. Используя __EmailVerificationSerializer__ и __VerifyEmail__.

http://0.0.0.0:8000/auth/register/ - адрес регистрации пользователей.

Авторизация зарегистрированных пользователей посредством реализации __LoginSerializer__ и __LoginAPIView__.

http://0.0.0.0:8000/auth/login/ - адрес для логина уже зарегистрированных пользователей.

http://0.0.0.0:8000/auth/token_refresh/ - адрес для __refresh токена__, возвращающий __acceess токен__.


## Statistic

Добавлены ещё три модельки, для каждого участника сделок, хранящие записи всех задействованных денег и всех проданных / купленных автомобилей. Отдельным таском планировщик запускает функцию, которая выводит в  celery-лог статистику по покупателю, автосалону и поставщику.


## Tests

Создаём конфиг для запуска Pytests указывая путь сборки, настройки, просматриваемые файлы и флаги запуска тестов.
